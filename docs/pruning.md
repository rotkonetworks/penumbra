...
```
systemctl stop penumbra          # pd must not be running; cometbft may stay up
pd migrate --home <pd_home> prune
systemctl start penumbra
```

Needs free disk for the pruned copy (about 10% of the current store) and a
raised open-file limit if run outside the systemd unit (`ulimit -n 65536`).
Measured on mainnet at version 12.56M: 350 GB to 30 GB in 59 minutes on one
core, 4.7 GB peak memory.
